...
通用的本地文档结构化工具：上传 PDF 或图片后，Worker 从 PDF 提取文字或使用本地 PaddleOCR 识别图片；Google Gemini 按用户定义模板提取严格 JSON，并保存到 SQLite，可在网页修改与导出 Excel。

## 架构

- **nginx**：唯一的 Web 入口，提供 Vue 静态文件并将 `/api/` 代理至 backend。
- **backend**：FastAPI API、SQLite 初始化、文件安全保存、模板/结果管理与 Excel 导出。
- **worker**：轮询 pending 文档，执行 PyMuPDF、PaddleOCR 3.1 CPU 和 Gemini。
...
```bash
cp .env.example .env
# 在 .env 中填写 GOOGLE_API_KEY，并按需修改 GOOGLE_MODEL
...
3. 点击“开始处理”。PDF 只通过 PyMuPDF 提取前两页的嵌入文字；没有文字的扫描 PDF 会失败（V1 不会隐式 OCR PDF）。图片由本地 PaddleOCR 处理。
